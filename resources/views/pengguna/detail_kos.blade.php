...
<div id="kos" class="kos-detail-container">
    ...
    <div class="info-section">
       ...
       @php
        ...
        @if ($sudahFavorit)
            <form action="{{ route('kos.unfavorit', $kos->id) }}" method="POST" style="display: inline;">
                @csrf
                <button type="submit" class="btn btn-danger d-flex align-items-center gap-2 px-3 py-2" style="border-radius: 8px;">
                    <i class="bi bi-heart-fill"></i> Hapus dari Favorit
                </button>
            </form>
        @else
            <form action="{{ route('kos.favorit', $kos->id) }}" method="POST" style="display: inline;">
                @csrf
                <button type="submit" class="btn btn-outline-danger d-flex align-items-center gap-2 px-3 py-2" style="border-radius: 8px;">
